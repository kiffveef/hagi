...
**`.claude/`はgitワークフローの外。編集したらそれで完了。commitは不要。**

**ワークフロー:**
```
.claude/を編集 → 完了(gitは関係ない)
同期が必要なら → hagi sync
```
...
**よくある間違い:**
`.claude/`を編集後、commitしようとする → gitignoreエラー → 混乱
→ 正解: commitしようとしないこと。編集=即反映。

**理由:**
- `hagi install`で生成されるローカル設定
- 開発者ごとに異なる設定
- `hagi sync`で別途管理
