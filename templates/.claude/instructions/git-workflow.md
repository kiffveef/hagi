...
**The `.claude/` directory is already gitignored and MUST NOT be committed.**

- ❌ **NEVER run `git add .claude/`**
- ❌ **NEVER use `git add --force` on `.claude/`**
- ❌ **NEVER commit `.claude/` files**

**WHY:**
- `.claude/` is generated by `hagi install` for local Claude Code configuration
- It contains session-specific settings that vary per developer
- Committing it causes unnecessary conflicts and clutters history
- The directory is intentionally in `.gitignore`

**If `git status` shows `.claude/` as untracked:**
- This is expected - `.gitignore` is working correctly
- Do NOT add it to git
- Simply ignore it and continue with your work
